...
func (this *GlobRequest) EqualMessageVT(thatMsg proto.Message) bool {
	that, ok := thatMsg.(*GlobRequest)
	if !ok {
		return false
	}
	return this.EqualVT(that)
...
func (this *ServerInfoResponse) EqualMessageVT(thatMsg proto.Message) bool {
	that, ok := thatMsg.(*ServerInfoResponse)
	if !ok {
		return false
	}
	return this.EqualVT(that)
...
func (this *ProtocolVersionResponse) EqualMessageVT(thatMsg proto.Message) bool {
	that, ok := thatMsg.(*ProtocolVersionResponse)
	if !ok {
		return false
	}
	return this.EqualVT(that)
...
func (m *FetchResponse) MarshalToSizedBufferVT(dAtA []byte) (int, error) {
	...
	if len(m.IsAbsent) > 0 {
		...
		i = protohelpers.EncodeVarint(dAtA, i, uint64(len(m.IsAbsent)))
		i--
		dAtA[i] = 0x32
	}
	if len(m.Values) > 0 {
...
func (m *InfoResponse) MarshalToSizedBufferVT(dAtA []byte) (int, error) {
	...
	if len(m.AggregationMethod) > 0 {
		...
		i = protohelpers.EncodeVarint(dAtA, i, uint64(len(m.AggregationMethod)))
		i--
		dAtA[i] = 0x12
	}
	if len(m.Name) > 0 {
...
func (m *FetchResponse) SizeVT() (n int) {
	...
	if len(m.IsAbsent) > 0 {
		n += 1 + protohelpers.SizeOfVarint(uint64(len(m.IsAbsent))) + len(m.IsAbsent)*1
	}
	n += len(m.unknownFields)
	return n
}
